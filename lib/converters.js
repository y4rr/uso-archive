...
function toUsercss(style) {
	...
	function processKey(key) {
		if (/^[\w-_]+$/.test(key)) {
			...
			replaces.push({ from: new RegExp(utils.escapeRegExp(`/*[[${key}]]*/`), "gi"), to: `/*[[${result}]]*/` });
			return result;
		}
	}
	...
	${toUsercssDropdownOptions(setting.options)}
		...
		case "image":
			return `@advanced dropdown ${processKey(setting.key)} "${setting.label.replace(/"/g, "\\\"")}" {
${toUsercssDropdownOptions(setting.options)}
	${processKey(setting.key)}-custom-dropdown "Custom" <<<EOT /*[[${processKey(setting.key)}-custom]]*/ EOT;
}
@advanced text ${processKey(setting.key)}-custom "${setting.label.replace(/"/g, "\\\"")} (Custom)" "https://example.com/image.png"`;
		case "text":
